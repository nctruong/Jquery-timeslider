    ...
    $.widget("ui.timeslider", {
        ...
        _slideTime: function (event, ui){
            ...
            if(that.slider( "option", "range" )){
				...
				that.timeslider('option', 'from_time').val(startTime)
				console.log("start time: " + that.slider("values", 0));
				that.timeslider('option', 'to_time').val(endTime)
				console.log("end time: " + that.slider("values", 1));
        ...
        _checkMax: function(event, ui) {
            ...
            if(that.slider( "option", "range" )){
                var div = that.find('div'),
					...
					console.log(that.slider("values", 1));
                // if( size >= 2160) {
				if( that.slider("values", 1) >= 2160) {
